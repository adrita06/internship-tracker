...
function HRContacts() {
  ...
  return <div><h1>HR Contacts</h1>{error && <p className="error-message">{error}</p>}{allContacts.length === 0 ? <p className="muted">No HR contacts found.</p> : <table><thead><tr><th>Name</th><th>Company</th><th>Email</th><th>Mobile</th></tr></thead><tbody>{allContacts.map((contact) => <tr key={contact._id || `${contact.companyName}-${contact.email}`}><td>{contact.name}</td><td>{contact.companyName}</td><td>{contact.email}</td><td>{contact.mobile || "-"}</td></tr>)}</tbody></table>}</div>;
}
